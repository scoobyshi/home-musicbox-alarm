...
musicService.prototype._scheduleEvents = scheduler.initialSchedule;
...
musicService.prototype.playlists = function(responsecb) {
  ...
  // Should be using asList() vs getPlaylists() since Mopidy v1.0?
  // returns .name and .uri which is spotify uri

  this.mopidy.playlists.asList().then( (playlists) => {
    if (playlists) {
...
musicService.prototype.getTracklistCount = function(responsecb) {
  console.log("Recieved Mopidy Request for the tracklist count");
  this.mopidy.tracklist.getLength().then( (tracklistcount) => {
    if (tracklistcount) {
      console.log(tracklistcount);
      responsecb.json({ "count": tracklistcount });
    } else {
      responsecb.status(400).json({ "error": "no tracklist count available" });
    }
  })
  .catch( () => res.status(500).json({ "error": "unknown" }))
  .done();
...
musicService.prototype.getTracklist = function(responsecb) {
  console.log("Recieved Mopidy Request for the tracklist");
  this.mopidy.tracklist.getTracks().then( (tracklist) => {
    if (tracklist) {
      console.log(tracklist);
      responsecb.json(tracklist);
    } else {
      responsecb.status(400).json({ "error": "no tracklist queue available" });
    }
  })
  .catch( () => res.status(500).json({ "error": "unknown" }))
  .done();
...
musicService.prototype.shuffle = function(responsecb) {
  console.log("Recieved Mopidy Request to Shuffle Tracklist");
  this.mopidy.tracklist.shuffle().then( () => this._sendResponse(200, responsecb))
    .catch( () => this._sendResponse(500, responsecb))
    .done();
}
