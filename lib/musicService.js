...
musicService.prototype._scheduleEvents = function () {
  ...
  db.all("SELECT id, playlistid, cron FROM playschedule", function(err, rows) {
    ...
    schedule.scheduleJob(myScheduleCron, () => {
      ...
      that.mopidy.tracklist.clear();
      that.mopidy.tracklist.add(null, null, myPlaylist)
        .then( () => console.log("Loaded Playlist:", myPlaylist))
        .catch( () => console.log("Failed to load scheduled playlist"))
        .done();

      // Finally play
      that.mopidy.playback.play()
        .then( () => console.log("Playing on Schedule"))
        .catch( () => console.log("Failed to play on Schedule"))
        .done();
    });
  ...
  switch (action) {
    case 'play':
      this.mopidy.playback.play()
        ...
        .catch( () => responsecb.status(500).json({ "error": "unknown" }))
        .done();
      break;
    case 'next':
